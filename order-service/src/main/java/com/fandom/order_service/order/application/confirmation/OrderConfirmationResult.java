 ...
 *
 * - CONFIRMED: PAID → CONFIRMED 전이 완료. notification.send(ORDER_COMPLETED) 발행 대상.
 * - ALREADY_CONFIRMED: Kafka at-least-once 재전송 등으로 동일 이벤트가 중복 수신된 경우.
 *   변경 없이 멱등 처리하고 이벤트는 재발행하지 않는다.
 * - SKIPPED_INVALID_STATE: PAID가 아닌 상태(예: 그 사이 유저가 취소해 REFUND_REQUESTED로 빠진 경우)에서
 *   늦게 도착한 이벤트. 예외를 던지지 않고 로그만 남기고 스킵한다 — 이미 다른 경로로 종료된 주문에
 *   대해 Kafka 재시도를 유발할 이유가 없다.
 */
