 ...
 * 전달된다.
 *
 * - REFUND_REQUESTED_STARTED: CONFIRMING/CONFIRMED → CANCEL_REQUESTED 전이 완료. 락 밖에서 비동기 PG
 *   환불 요청이 필요하며, paymentToRefund/userId에 담긴 값으로 진행한다. 실제 환불 완료/거절 결과는
 *   PG 웹훅으로 비동기 반영되므로(RefundResultWriter 참고) 이 Writer는 더 이상 최종 결과
 *   (CANCELLED/FAILED)를 직접 만들지 않는다.
...
public record OrderCompensationResult(
    ...
    public static OrderCompensationResult refundRequestedStarted(UUID orderId, Payment paymentToRefund, UUID userId) {
        return new OrderCompensationResult(
                Type.REFUND_REQUESTED_STARTED, orderId, OrderStatus.CANCEL_REQUESTED, paymentToRefund, userId);
    }
