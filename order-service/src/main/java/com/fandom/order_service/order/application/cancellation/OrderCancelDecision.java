 ...
 * - IDEMPOTENT: 이미 종료 상태(CANCELLED/REFUNDED/FAILED 등)인 주문 재요청. 변경 없이 현재 상태 그대로.
 * - REFUND_NEEDED: PAID/CONFIRMED → REFUND_REQUESTED 전이 완료. 락 밖에서 비동기 PG 환불 요청이
 *   필요하며, paymentToRefund에 담긴 pg_transaction_id로 환불을 요청한다. 실제 환불 완료/거절 결과는
 *   PG 웹훅으로 비동기 반영되므로, 이 결과를 받은 직후 응답은 REFUND_REQUESTED 상태로 나간다.
 */
public record OrderCancelDecision(
    ...
    public static OrderCancelDecision refundNeeded(UUID orderId, Payment paymentToRefund, LocalDateTime updatedAt) {
        return new OrderCancelDecision(
                Type.REFUND_NEEDED, orderId, OrderStatus.REFUND_REQUESTED, updatedAt, paymentToRefund);
    }
}