...
class OutboxSchedulerTest {
    ...
    @Test
    @DisplayName("JSON 역직렬화 실패 3회 미만 - 재시도를 위해 PENDING으로 유지")
    void publishPendingEventsDeserializationFailureRetainsPending() throws Exception {
        // given
    ...
    @Test
    @DisplayName("JSON 역직렬화 실패 3회 이상 - FAILED로 마킹되어 재시도 멈춤")
    void publishPendingEventsFailsPermanentlyAfterThreeRetries() throws Exception {
        // given
        UUID postId = UUID.randomUUID();
        UUID authorId = UUID.randomUUID();
        String json = "{\"postId\":\"...\"}";
