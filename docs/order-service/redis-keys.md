...
> ⚠️ **공유 Redis 인스턴스**: order-service의 분산락/멱등성 캐시는 자체 Redis가 아니라 **ticketing-service와 같은 Redis 인스턴스(포트 6380)를 공유**한다(`order-service.yml`의 `spring.data.redis.host/port`가 `REDIS_TICKETING_HOST/PORT`를 가리킴). 즉 이 Redis가 죽으면 order-service의 4개 키 전부와 ticketing-service의 대기열/좌석 점유 로직이 동시에 영향받는다 — 아래 "장애 시 동작" 표는 이 공유 인스턴스가 완전히 죽었을 때를 기준으로 한다.

> TTL 등 아래 설정값은 `order-service.yml`(Config Server) 기준으로 확인됨(2026-07).
...
| `cacheTtlSeconds` | 600초(10분) | INSERT 성공 후 결과 캐시 TTL. `order-service.yml`에 "Ticketing 좌석 선점 TTL과 맞출 것"이라는 TODO 주석이 달려 있어, Ticketing 쪽 실제 선점 TTL 확인 후 조정 필요 |
...
```
`DROP` 없이 `CREATE INDEX IF NOT EXISTS`만 쓰면 기존 인덱스가 있을 때 `WHERE` 조건 변경이 조용히 무시된다. Redis 장애 시 1차 방어를 건너뛰고 바로 INSERT를 시도하며, 이 부분 UNIQUE 인덱스 충돌로 최종 차단한다. 충돌 시 Redis 클레임을 보상 삭제(DEL)한 뒤 DB에서 실제 진행중 주문을 찾아 멱등 응답으로 돌려준다.
...
락은 "결제 레코드(payments REQUESTED) 생성 + latest_payment_id 갱신, 커밋"까지만 감싸고 풀린다(`orders.status`는 계속 PENDING). PG 호출(`requestApprovalAndCache`)은 락 밖에서 수행된다.
- 락이 풀린 뒤 들어오는 동시 요청은 이미 `payments`에 `REQUESTED` 레코드가 존재하므로 진행중 여부 검증에서 자연히 거부된다.
...
**락 획득 순서**
1. 멱등성 키 조회 (락 밖, early check) — 캐시 히트면 락을 잡지도 않고 바로 반환. 중복 요청의 흔한 경로(같은 키 재시도)에서 락 경합 자체를 피하기 위함
2. Redis 분산락 획득
3. 멱등성 키 재확인 (락 안에서 — 1번과 2번 사이의 경쟁 상황 대비)
4. DB 비관적 락(`SELECT FOR UPDATE`) + 상태 검증 + 상태 전이 + 커밋
5. 분산락 해제
6. PG 비동기 승인 요청 (락 밖)
7. 결과 캐싱
...
| `payment:lock:order:*` | `tryLock()`이 `InterruptedException`만 잡고 있어 Redis(Redisson) 연결 예외는 그대로 전파됨 → 요청 실패(500). 다른 3개 키처럼 "1차 생략 후 계속 진행"하는 폴백이 없음 | 없음 (미흡) |