...
주문 취소 요청
→ 주문 CANCELLED
→ order.payment.cancelled 발행 (좌석 선점 해제)
→ 응답: 200 CANCELLED
```
...
sequenceDiagram
    ...
    O->>K: order.payment.cancelled (좌석 선점 해제)
    K->>T: 좌석 해제 처리
    O-->>C: 200 CANCELLED
```
...
```
스케줄러 실행 (expired_at 기준)
→ PENDING 주문 → CANCELLED + order.payment.cancelled 발행
→ PAYMENT_REQUESTED 주문 → PG 거래 조회 후 처리
   ├─ 실제 승인됨 → PAID 전이 후 정상 흐름 진행
   └─ 미승인 → CANCELLED
```