...
- Gateway의 인증 전 요청 보호, 장애 격리, traceId 전파
...
- Feed 서비스와 User Service 사이의 내부 Follow 조회 흐름
...
| 영역 | 주요 책임 |
| --- | --- |
| Gateway | 외부 요청 진입점, Access Token 검증, 인증 제외 경로 관리, 내부 인증 헤더 생성, rate limit, 장애 격리, traceId 전파 |
| common-auth | 내부 서비스에서 인증 컨텍스트를 일관되게 검증하고 주입하기 위한 공통 모듈 |
| Auth Service | 로그인, Access Token 및 Refresh Token 발급, 재발급, 로그아웃, 토큰 무효화 |
| User Service | 회원/크리에이터 가입, 회원 정보, 프로필, 팔로우, 회원 탈퇴, 사용자 이벤트 발행, Feed 연동용 Follow 내부 조회 API 제공 |
...
2. Gateway는 로그인 경로를 인증 예외 경로로 통과시키되, 인증 전 요청 rate limit을 먼저 적용한다.
...
2. Gateway는 재발급 경로에 인증 전 요청 rate limit을 적용한다.
3. Auth Service는 Refresh Token의 유효성을 검증한다.
4. Redis에 저장된 Refresh Token과 요청 토큰을 비교한다.
5. 유효하면 새로운 Access Token을 발급한다.
6. 사용자 단위 blacklist가 존재하면 재발급을 거부한다.
...
## 4. Gateway 인증 및 운영 안정화 처리

Gateway는 외부 요청의 인증과 내부 인증 컨텍스트 생성을 담당한다. 또한 인증 전 과호출 방어, downstream 장애 격리, traceId 전파를 담당한다.
...
### 4.2 인증 전 요청 rate limit

로그인과 토큰 재발급은 인증 제외 경로이지만, brute-force 및 비정상 반복 요청의 주요 표적이다.

Gateway는 다음 경로에 Redis Token Bucket 기반 rate limit을 적용한다.
...
- 인증 전 요청이므로 `IP + path` 기준으로 버킷을 분리한다.
- 제한 초과 시 `429 + ApiResponse` 형식으로 응답한다.
- 초기 제한값은 `replenishRate=5`, `burstCapacity=10`, `requestedTokens=1`이다.
- 부하테스트 결과에 따라 임계값은 조정할 수 있다.
...
### 4.5 CircuitBreaker / timeout / fallback

Gateway는 downstream 서비스 장애가 전체 요청 흐름으로 전파되는 것을 줄이기 위해 주요 route에 CircuitBreaker와 timeout을 적용한다.

정책:

- 주요 downstream route별 CircuitBreaker instance를 둔다.
- CircuitBreaker open 또는 downstream 호출 실패 시 `/fallback`으로 forward한다.
- `FallbackController`는 fallback 원인을 `CIRCUIT_OPEN` / `TIMEOUT` / `NO_INSTANCE_AVAILABLE` / `CONNECTION_FAILED` / `DOWNSTREAM_ERROR` / `UNKNOWN`으로 분류하고, 응답에 `X-Fallback-Reason`·`X-Fallback-Route` 헤더를 담는다.
- 원인이 `TIMEOUT`이면 의미상 맞는 **504**를, 그 외(CB open, 인스턴스 부재 등)는 **503**을 반환한다. CircuitBreaker가 TimeLimiter와 묶여 동작하므로 CB가 적용된 라우트에선 타임아웃도 이 fallback을 통해 처리된다.
- fallback·timeout 응답 모두 공통 `ApiResponse` 형식이며, 현재 trace가 있으면 `X-Trace-Id` 헤더를 포함한다.
...
### 4.6 Trace / access log

Gateway는 WebFlux tracing context를 전파하고, 요청별 access log를 남긴다.
...
`X-Trace-Id`는 클라이언트 문의, Gateway 로그, downstream 서비스 로그, Zipkin trace를 연결하는 기준으로 사용한다.
...
`Profile.followerCount`와 `Profile.followingCount`는 팔로우/언팔로우 시 갱신되는 집계값이며, Feed 내부 API에서 팬아웃 여부와 대형 크리에이터 여부를 빠르게 판단하는 데 사용한다.
...
### 7.5 Feed 연동용 내부 Follow 조회

Feed 서비스는 팬아웃 및 타임라인 조회를 위해 User Service의 내부 Follow 조회 API를 사용한다.
...
| API | 용도 |
| --- | --- |
| `GET /internal/v1/follows/{authorId}/count` | 팬아웃 여부 판단용 팔로워 수 조회 |
| `GET /internal/v1/follows/{authorId}/followers` | 팬아웃 대상 follower userId 조회 |
| `GET /internal/v1/follows/{userId}/followings` | 타임라인 구성용 팔로잉 목록 조회 |
| `GET /internal/v1/follows/{userId}/followings/large` | 대형 크리에이터 팔로잉만 조회 |

공통 정책:

- 내부 API는 Gateway 라우팅 대상이 아니다.
- 서비스 간 직접 호출 전용이다.
- 응답은 `ApiResponse`와 `CursorPageResponse<T>`를 사용한다.
- 커서는 `follow.id` 기준이다.
- `size` 허용 범위는 1 이상 1000 이하다.
- 대형 크리에이터 기준은 `Profile.followerCount > minFollowerCount`다.
...
`user.deleted`는 Auth Service만이 아니라 **chat / notification 서비스도 함께 소비**한다(Auth: 토큰 무효화 / chat: 채팅 정리 / notification: 알림 정리). 또한 회원 탈퇴 1회에 `user.deleted`와 함께 역할별 이벤트(`user.member-withdrawn` 또는 `user.creator-withdrawn`, feed 소비)가 같은 트랜잭션에서 함께 발행된다. 전체 이벤트 명세는 [user-events.md](./user-events.md)를 따른다.
...
| `user.followed` | `follow_id` | `{ "follow_id": "uuid", "follower_id": "uuid", "followee_id": "uuid", "nickname": "name" }` | 팔로우 발생 시 채팅방 입장 |
...
발행 신뢰성을 위해 **Transactional Outbox 패턴**을 적용한다. 도메인 상태 변경과 Outbox 레코드 INSERT를 같은 트랜잭션으로 커밋하고, 별도 폴링 relay가 커밋된 Outbox를 읽어 Kafka로 발행한다(at-least-once). 중복은 consumer 멱등성으로 흡수한다. 상세는 [user-events.md](./user-events.md)를 따른다. (이전에는 서비스 로직 내 직접 발행이라 "커밋은 됐는데 발행 실패" 시 이벤트 유실 위험이 있어 전환했다.)
...
| API | 인증 | 설명 |
| --- | --- | --- |
| `POST /api/v1/auth/login` | 불필요 | 로그인, Gateway rate limit 적용 |
| `POST /api/v1/auth/reissue` | 불필요 | Access Token 재발급, Gateway rate limit 적용 |
...
| API | 호출 주체 | 설명 |
| --- | --- | --- |
| `GET /internal/v1/follows/{authorId}/count` | Feed | 팬아웃 여부 판단용 팔로워 수 조회 |
| `GET /internal/v1/follows/{authorId}/followers` | Feed | 팬아웃 대상 follower userId 목록 조회 |
| `GET /internal/v1/follows/{userId}/followings` | Feed | 타임라인 조회용 팔로잉 목록 조회 |
| `GET /internal/v1/follows/{userId}/followings/large` | Feed | 대형 크리에이터 팔로잉 목록 조회 |
...
로그인과 토큰 재발급은 인증 제외 경로이지만, Gateway rate limit으로 과도한 반복 요청을 제한한다.

운영에서 Gateway가 프록시/ALB 뒤에 놓이므로, rate limit KeyResolver는 `XForwardedRemoteAddressResolver.maxTrustedIndex(1)`로 실제 클라이언트 IP를 추출한다(ALB가 확인한 오른쪽 1홉만 신뢰, 클라이언트 위조 헤더 무시). 로컬(X-Forwarded-For 없음)에서는 remoteAddress로 자동 fallback된다.
...
- 프로필 수정 검증
- 팔로우/언팔로우 중복 및 권한 검증
- 내부 Follow API의 커서 페이징 및 size 검증
- 토큰 재발급 및 무효화 검증
- Gateway rate limit key 생성 및 429 응답 검증
- Gateway fallback/timeout 응답 검증
...
- 로그인/재발급 과호출 시 429 반환
- downstream 장애 시 503 fallback 반환
- timeout 시 504 반환
...
docker exec 6pm-redis-general redis-cli -a fandom_redis_pw TTL "blacklist:user:${userId}"
```
...
Gateway 응답 헤더와 로그에서 traceId를 확인한다.

확인 대상:

- Gateway 응답 헤더 `X-Trace-Id`
- Gateway access log의 traceId/spanId
- downstream MVC 서비스 access log의 traceId/spanId
- Zipkin trace
...
- 이벤트 소비 멱등성 저장소 정책 (feed는 ProcessedEvent로 구현 완료, 나머지 consumer 확장)
- DLQ 및 재처리 정책 구현
- Gateway RBAC 고도화
- 인증 선택 API에서 optional id card 처리 방식 정리
- 부하테스트 결과 기반 rate limit 임계값 조정
- SSE/streaming route timeout 분리 여부 검토
- 운영 알림과 모니터링 기준 정리
