...
- Gateway HTTP client timeout은 `504 + ApiResponse` 형식으로 반환한다.
- 응답에 현재 trace가 있으면 `X-Trace-Id` 헤더를 포함한다.
...
운영에서 Gateway가 프록시/ALB 뒤에 놓이는 경우 실제 클라이언트 IP 기준 제한을 위해 `X-Forwarded-For` 처리 정책을 별도로 검토한다.
