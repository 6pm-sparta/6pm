...
| `releaseHold()` → order-service 주문 취소 연동 끊김 | 🔴 order 승인 대기 | `DELETE /internal/v1/orders/{orderId}` 엔드포인트가 develop `InternalOrderController`에 아직 없음(order 파트 승인 대기 중이라 이번 브랜치에서 관련 변경 제외). `SeatService.releaseHold()`의 `orderClientRetryWrapper.cancel()` 호출이 현재 항상 404 → 폴백으로 조용히 삼켜짐. order-service 쪽 주문 취소는 타임아웃 자동취소(#231)에만 의존. order 담당자가 엔드포인트 추가/승인하면 해소 |
