...
- [x] `QueueScheduler`에 분산 락 없음 (완료 2026-07-05)
  - 멀티 인스턴스 환경에서 같은 배치를 중복 처리할 수 있음
  - `issue()`(SETNX)/`ZREM`이 멱등이라 데이터 정합성은 깨지지 않고 로그 중복/SSE 중복 호출 정도의 비효율만 발생 → 우선순위 낮음
  - Redisson `RLock`으로 `processQueue()` 전체를 감싸 인스턴스당 한 번에 하나만 실행되도록 수정. 활성 공연 수에 따라 처리 시간이 가변적이라 고정 `leaseTime` 대신 워치독(자동 갱신) 방식 사용(`tryLock(0, TimeUnit.SECONDS)`)
