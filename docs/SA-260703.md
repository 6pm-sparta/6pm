# 6pm 팬덤 티켓팅 플랫폼 — 시스템 분석서 (핵심 흐름: Auth · User · Ticketing · Order)
...
| POST | `/api/v1/tickets/shows/{showId}/seats/{seatId}/hold` | 필요 | 좌석 선점 + 주문 생성 트리거 (`purchase-token` 검증 선행) |
...
```json
// POST .../seats/{seatId}/hold → 201
{ "status": 201, "message": "CREATED", "data": { "orderId": "..." } }
...
## 11. 예매(체크아웃) API 분리 — 설계 노트 (2026-07-03)

### 배경
`SeatService.hold()`가 좌석 선점(Redis)과 주문 생성(order-service 동기 Feign 호출)을 한 트랜잭션에서 처리 중([SeatService.java:128-137](../ticketing-service/src/main/java/com/fandom/ticketing_service/seat/application/SeatService.java)). 원래 "부하 우려로" 예매 API 시점 생성에서 hold 시점 생성으로 리팩토링된 결과인데, 실제로는 가장 트래픽이 몰리는 구간(hold)에 동기 cross-service 호출을 넣은 역효과일 가능성이 높다.

### 방향
- `hold()`에서 주문 생성 호출 제거, 별도 예매(체크아웃) API로 트리거 이전
- 체크아웃 API는 **동기 유지** — 실제 병목(좌석 hold)은 이미 Redis Lua가 해결하고, 체크아웃은 구매토큰 큐를 통과한 인원만 호출하는 지점이라 물량이 이미 필터링돼있음. 비동기 전환은 성능 이득 없이 폴링/SSE 등 복잡도만 늘림
- 구매토큰 검증 게이트, Rate Limit(hold와 동일 기준)을 체크아웃 API에도 동일 적용
- **순수 분리 스코프로 시작** (다중예매 번들링 여부와 무관하게 착수 가능) — 이 스코프에서는 order-service API 계약 변경 없음
